...
Like with the [VGA text buffer][vga lazy-static], we use `spin::Lazy` and a spinlock to create a `static` writer instance. By using `Lazy` we can ensure that the UART is initialized exactly once on its first use.
