...
上一篇[文章]已经对分页的概念做了介绍。通过比较分页和分段来证明分页的优势，然后解释了分页和页表如何工作，最后介绍了`x86_64`的4级页表设计。我们发现 bootloader 已经为内核建立了一个页表层次结构，这意味着我们的内核已经在虚拟地址上运行。这这样做提高了安全性，因为非法的内存访问会导致页面故障异常，而不是修改任意的物理内存。

[前文]: @/edition-2/posts/08-paging-introduction/index.md

这篇文章最后说，我们[不能从我们的内核中访问页表][end of previous post]，因为它们存储在物理内存中，而我们的内核已经在虚拟地址上运行。

这篇文章探讨了使页表框能够被内核访问的不同方法。我们将讨论每种方法的优点和缺点，然后为我们的内核决定一种方法。
...
- `map_physical_memory` 功能将某处完整的物理内存映射到虚拟地址空间。因此，内核可以访问所有的物理内存，并且可以遵循[_映射完整的物理内存_](#map-the-complete-physical-memory)的方法。
...
正如我们在[_Boot Information_](#boot-information)部分所解释的，内存图是由 BIOS/UEFI 固件提供的。它只能在启动过程的早期被查询，所以引导程序已经为我们调用了相应的函数。内存地图由[`MemoryRegion`]结构列表组成，其中包含每个内存区域的起始地址、长度和类型（如未使用、保留等）。
...
此时，我们应该再次删除`create_example_mapping`函数，以避免意外地调用未定义的行为，正如[上面](#a-create-example-mapping-function)所解释的那样。
