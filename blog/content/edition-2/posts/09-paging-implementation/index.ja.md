...
- この物理メモリオフセットは、Cargo.tomlに`[package.metadata.bootloader]`テーブルを追加し、`physical-memory-offset = "0x0000f00000000000"`（あるいは他の任意の値）というフィールドを設定することでカスタマイズできます。ただし、ブートローダは、オフセットより先の空間、つまり以前に他の初期の物理アドレスにマッピングしていた領域と重なり始めるような物理アドレス値に遭遇すると、パニックすることがあるので注意してください。したがって一般的には、この値は大きければ大きい（> 1 TiB）ほど良いです。
