...
[end of previous post]: @/edition-2/posts/08-paging-introduction/index.ru.md#доступ-к-таблицам-cтраниц

Для реализации выбранного подхода нам потребуется поддержка загрузчика, поэтому мы сначала настроим его. После этого мы реализуем функцию, которая будет обходить иерархию таблицы страниц для преобразования виртуальных в физические адреса. Наконец, мы изучим, как создавать новые маппинги (mappings) в таблицах страниц и как находить незадействованные фреймы памяти для создания новых таблиц страниц.
...
![Привер иерархии 4-уровневой таблицы страниц, где каждая таблица показана в физической памяти](../paging-introduction/x86_64-page-table-translation.svg)

Важно то, что каждая запись страницы (page entry) хранит _физический_ адрес следующей таблицы. Это избавляет от необходимости выполнять трансляцию и для этих адресов, что было бы плохо для производительности и могло легко вызвать бесконечные циклы трансляции.

Наша проблема в том, что мы не можем напрямую обращаться к физическим адресам из нашего ядра, поскольку само ядро работает поверх виртуальных адресов. Например, когда мы обращаемся к адресу `4 КБ`, мы обращаемся к виртуальному адресу 4 КБ, а не к физическому адресу `4 КБ`, где хранится таблица страниц 4-го уровня. Когда нам нужен доступ к физическому адресу `4 КБ`, мы можем сделать это только через какой-либо виртуальный адрес, который на него мапятся.

Таким образом, чтобы получить доступ к фреймам таблиц страниц, нам необходимо сопоставить им некоторые виртуальные страницы. Существует разные способы создания этих маппингов, и все они позволяют нам обращаться к произвольным фреймам таблиц страниц.

### Идентичное Маппинг

Простое решение - **идентично отообразить все таблиц страниц**:

![Виртуальныое и физическое адресное пространство с отображением на физические фреймы на те же адреса](identity-mapped-page-tables.svg)

В этом примере мы видим различные фреймы таблицы страниц с идентичным отображением. Таким образом, физические адреса таблицы страниц становятся одновременно и допустимыми виртуальными адресами, что позволяет нам легко получать доступ к таблицам всех уровней, начиная с регистра CR3.
...
[сегментации]: @/edition-2/posts/08-paging-introduction/index.ru.md#фрагментация
...
### Маппинг с Фиксированным Смещением

Чтобы избежать перегрузки виртуального адресного пространства, мы можем **использовать отдельный участок памяти для отображения таблиц страниц**. Таким образом, вместо идентичного отображения фреймов таблицы страниц мы отображаем их с фиксированным смещением в виртуальном адресном пространстве. Например, смещение может составлять 10&nbsp;ТиБ:

![Тот же рисунок, что и для идентичного маппинга, но каждая отображенная виртуальная страница смещена на 10 ТиБ.](page-tables-mapped-at-offset.svg)
...
![Аналогичная маппингу со смещением, но теперь каждая физическая ячейка имеет карту (в диапазоне 10 ТиБ + X), а не только ячейки таблиц страниц.](map-complete-physical-memory.svg)

Такой подход позволяет нашему ядру обращаться к произвольной физической памяти, включая ячейки таблиц адресации других пространств адресов. Выделенный диапазон виртуальной памяти сохраняет прежний размер — главное отличие в том, что он больше не содержит некартированных страниц.  

Недостаток этого подхода заключается в необходимости дополнительных таблиц страниц для хранения карты физической памяти. Эти таблицы должны где-то храниться, поэтому они занимают часть физической памяти, что может стать проблемой на устройствах с ограниченным объемом памяти.  

Однако на архитектуре x86_64 мы можем использовать [huge-pages] размером 2&nbsp;МБ для картирования вместо стандартных страниц по 4&nbsp;КБ. Таким образом, для карты 32&nbsp;ГБ физической памяти потребуется всего 132&nbsp;КБ под таблицы страниц: достаточно одной таблицы третьего уровня и 32 таблиц второго уровня. Huge-pages также более эффективны в плане кэширования, так как используют меньше записей в буфер ассоциативной трансляции (Translation Lookaside buffer, TLB).

[huge-pages]: https://en.wikipedia.org/wiki/Page_%28computer_memory%29#Multiple_page_sizes

### Временный Маппинг

Для устройств с обчень малым объемом физической памяти мы могли бы **отображать фреймы таблицы страниц только временно**, когда нам нужно получить к ним доступ. Чтобы иметь возможность временно мапить памяти, нам нужна только одна идентично замапленная таблица первого уровня:

![Виртуальное и физическое адресное пространство с таблицей 1-го уровня с идентичным маппингом, которая сопоставляет свою 0-ю запись с фреймом таблицы 2-го уровня, тем самым сопоставляя этот фрейм со страницей с адресом 0](temporarily-mapped-page-tables.svg)

Таблица первого уровня на рисунке контролирует первые 2&nbsp;МБ виртуального вдресного пространства. Это потому, что она достижима, начиная с регистра CR3 и следуя 0-й записи в таблицах страниц уровня 4, 3 и 2. Запись с индексом `8` мапит виртуальную страницу по адресу `32 КБ` на физический фрейм с адресом `32 КБ`, тем самым идентично отображая саму таблицу первого уровня. Графика показывает это идентичное отображение горизонтальной стрелкой на `32 КБ`.
...
Еще один интересный подход, который вообще не требует дополнительных таблиц страниц, это **рекурсивный маппинг таблицы страниц**. Идея в том, чтобы замапить запись из таблицы страниц 4-го уровня на эту же таблицу. Таким образом, мы эффективно резервируем часть виртуального адресного пространства и отображаем все текущие и будущие фреймы таблицы страниц в это пространство.

Давайте рассмотрим пример, чтобы понять, как все это работает:

![Пример 4-уровневой иерархии страниц, где каждая таблица страниц отображена в физической памяти. Запись 511 страницы 4-го уровня отображается в фрейм 4 КБ, фрейм самой таблицы 4-го уровня.](recursive-page-table.png)
...
[примера в начале этого поста]: #доступ-к-таблицам-страниц

Позволяя ЦП следовать этой записи при трансляции, он не достигнет таблицы 3-го уровня, а останется на таблице уровня 4. Это похоже на рекурсивную функцию, которая вызывает саму себя, поэтому эта таблица называется _рекурсивной таблицей страниц_. Важно то, что ЦП предполагает, что каждая запись в таблице уровня 4 указывает на таблицу уровня 3, поэтому теперь он рассматривает таблицу уровня 4 как таблицу уровня 3. Это работает, потому что таблицы всех уровней имеют одинаковую структуру на x86_64.
...
<!-- TODO: `from CR4 to level 4 table` помарака? -->
...
<!-- TODO: `from CR4 to level 4 table` CR4 or CR3 ? -->
...
Наконец, мы можем получить доступ к таблице 4-го уровня, сдвинув каждый блок на четыре блока вправо и использовав рекурсивный индекс для всех блоков адресов, за исключением смещения:
...
[sign extension]: @/edition-2/posts/08-paging-introduction/index.ru.md#пагинация-на-x86-64
...
Для построения таких адресов в коде Rust можно использовать побитовые операции:
...
- Функция `map_physical_memory` отображает всю физическую память в виртуальное адресное пространство. Таким образом, ядро получает доступ ко всей физической памяти и может использовать подход [_Отображение всей физической памяти_](#отображение-всей-физической-памяти).
- С помощью функции `recursive_page_table` загрузчик рекурсивно отображает запись таблицы страниц 4-го уровня. Это позволяет ядру получать доступ к таблицам страниц, как описано в разделе [_Рекурсивные таблицы страниц_](#рекурсивные-таблицы-страницрдд).
...
Если этот функционал включен, загрузчик сопоставляет всю физическую память какому-либо неиспользуемому диапазону виртуальных адресов. Чтобы сообщить ядру о диапазоне виртуальных адресов, загрузчик передаёт структуру _загрузочные данные_.
<!-- TODO: boot information - загрузочные данные ? -->
...
[конце предыдущего поста]: @/edition-2/posts/08-paging-introduction/index.ru.md#доступ-к-таблицам-страниц
...
Мы видим, что здесь присутствуют различные непустые записи, каждая из которых сопоставляется с отдельной таблицей 3-го уровня. Регионов так много, потому что код ядра, стек ядра, отображение физической памяти и информация о загрузке используют отдельные области памяти.
...
Прохождение таблиц страниц вручную интересно тем, что помогает понять, как ЦП выполняет преобразование. Однако в большинстве случаев нас интересует только сопоставленный физический адрес для данного виртуального адреса, поэтому давайте создадим для этого функцию.

### Трансляция Адресов

Чтобы преобразовать виртуальный адрес в физический, нам нужно пройти по четырёхуровневой таблице страниц, пока не дойдём до соответствующего блока. Давайте создадим функцию, которая будет выполнять это преобразование:
...
[_page offset_]: @/edition-2/posts/08-paging-introduction/index.ru.md#пагинация-на-x86-64
...
/// Создает пример маппинга для полученой страница к фрейму `0xb8000`.
...
Помимо `page`, которую необходимо отобразить, функция ожидает мутабельную ссылку на экземпляр `OffsetPageTable` и `frame_allocator`. Параметр `frame_allocator` использует синтаксис [`impl Trait`][impl-trait-arg], указывает что это [дженерик] и принимает любой тип реализующий трейт [`FrameAllocator`]. Трейт является дженериком для трейта [`PageSize`], чтобы работать как со стандартными страницами размером 4&nbsp;КБ, так и с огромными страницами размером 2&nbsp;МБ/1&nbsp;ГБ. Нам нужно создать только отображение размером 4&nbsp;КБ, поэтому мы устанавливаем общий параметр в значение `Size4KiB`.
...
[_Формат таблицы страниц_]: @/edition-2/posts/08-paging-introduction/index.ru.md#формат-таблицы-страниц
...
[page-table-indices]: @/edition-2/posts/08-paging-introduction/index.ru.md#пагинация-на-x86-64
...
Для вызова нашей функции `create_example_mapping` с `EmptyFrameAllocator` нам нужно выбрать страницу, для которой уже существуют все таблицы страниц. Чтобы найти такую страницу, мы можем воспользоваться тем фактом, что загрузчик загружает себя в первый мегабайт виртуального адресного пространства. Это означает, что для всех страниц в этой области существует действительная таблица уровня 1. Таким образом, мы можем выбрать любую неиспользуемую страницу в этой области памяти для нашего примера отображения, например страницу по адресу `0`. Обычно эта страница должна оставаться неиспользованной, чтобы гарантировать, что разыменование нулевого указателя вызовет ошибку страницы, поэтому мы знаем, что загрузчик оставляет её несопоставленной.

#### Создаем Сопоставление

Теперь у нас есть все необходимые параметры для вызова нашей функции `create_example_mapping`, поэтому давайте изменим нашу функцию `kernel_main`, чтобы сопоставить страницу по виртуальному адресу `0`. Поскольку мы сопоставляем страницу с фреймом текстового буфера VGA, впоследствии мы сможем записывать данные на экран через неё. Реализация выглядит следующим образом:
...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    ...
    // map на неиспользуемую страницу
    let page = Page::containing_address(VirtAddr::new(0));
    memory::create_example_mapping(page, &mut mapper, &mut frame_allocator);

    // пишем строчку `New!` на экран через новый маппинг
...
Сначала мы создаём отображение для страницы по адресу `0`, вызвав нашу функцию `create_example_mapping` с мутабельной ссылкой на экземпляры `mapper` и `frame_allocator`. Это отображает страницу на фрейм текстового буфера VGA, поэтому любая запись в него должна отображаться на экране.
...
Чтобы сопоставить страницы, для которых ещё не создана таблица страниц первого уровня, нам нужно создать подходящий объект `FrameAllocator`. Но как узнать, какие фреймы не используются и сколько физической памяти доступно?
...
Прежде чем реализовать трайт `FrameAllocator`, добавим вспомогательный метод, преобразующий карту памяти в итератор доступных фреймов:
...
Эта функция использует методы комбинаторов итераторов для преобразования исходной `MemoryMap` в итератор доступных физических фреймов:
...
С помощью алгоритма выделения ячеек для блока загрузочной информации сопоставление выполняется успешно, и на экране снова появляется надпись _"New!"_ чёрным шрифтом на белом фоне. В фоновом режиме метод `map_to` создаёт недостающие таблицы страниц следующим образом:
...
На данном этапе нам следует снова удалить функцию `create_example_mapping`, чтобы избежать случайного возникновения неопределённого поведения, как объяснялось [выше](#функция-create-example-mapping).
