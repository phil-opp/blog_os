...
0. **古いスタックポインタを保存する**: CPUはスタックポインタ（`rsp`）とスタックセグメント（`ss`）レジスタの値を読み取り、内部バッファに記憶しておきます。
...
3. **古いスタックポインタをプッシュする**: CPUはステップ0で読み取った`rsp`と`ss`の値をスタックにプッシュします。これにより、割り込みハンドラからリターンしてきたときにもとのスタックポインタを復元することが可能になります。
...
### IDTを読み込む
CPUがこの割り込みディスクリプタテーブル(IDT)を使用するためには、[`lidt`]命令を使ってこれを読み込む必要があります。`x86_64`クレートの`InterruptDescriptorTable`構造体には、そのための[`load`][InterruptDescriptorTable::load]というメソッドが用意されています。それを使ってみましょう：
