...
0. **이전의 스택 포인터 저장**: CPU가 스택 포인터 (`rsp`)와 스택 세그먼트 (`ss`) 레지스터의 값을 읽어와 내부 버퍼에 기억해 둡니다.
...
3. **이전의 스택 포인터 push**: CPU는 0단계에서 얻은 `rsp`와 `ss` 값을 스택에 push합니다. 이로써 인터럽트 처리 함수로부터 반환 시 이전의 스택 포인터를 복원할 수 있습니다.
