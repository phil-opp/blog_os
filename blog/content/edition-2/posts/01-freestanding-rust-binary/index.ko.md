...
이제 컴파일러는 `#[panic_handler]` 함수를 찾지 못하며, 표준 라이브러리 없이는 _되감기 (unwinding)_ 가 불가능하다고 알려줍니다. 다음 섹션들에서 두 오류를 모두 살펴보겠습니다.

<div class="note">

구버전 Rust 툴체인에서는 두 번째 오류가 `language item required, but not found: eh_personality`으로 나타나는데, 이는 `unwinding panics are not supported without std` 오류와 같은 원인을 가집니다.
...
Rust는 기본적으로 [패닉][panic]이 일어났을 때 [스택 되감기 (stack unwinding)][stack unwinding]를 통해 스택에 살아있는 모든 변수의 소멸자를 호출합니다. 이를 통해 사용 중이던 모든 메모리 리소스가 반환되고, 부모 스레드가 패닉에 대처한 후 계속 실행될 수 있게 합니다. 하지만 스택 되감기는 복잡한 과정으로 이루어지며 운영체제마다 특정한 라이브러리를 필요로 하고 (예: Linux는 [libunwind], Windows는 [structured exception handling]), 이 라이브러리들은 Rust 표준 라이브러리를 필요로 합니다. 그렇기에 우리의 `no_std` 운영체제 커널에서는 스택 되감기를 사용할 수 없습니다.

<div class="note">

구버전 Rust 툴체인에서 나타나는 `language item required, but not found: eh_personality` 오류 역시 스택 되감기와 관련이 있습니다. [`eh_personality` language item]은 스택 되감기를 구현하는 데 사용되는 함수를 가리킵니다. Language item은 컴파일러가 내부적으로 요구하는 특별한 아이템 (트레잇, 함수, 타입 등)입니다. 신버전 Rust 툴체인에서는 이러한 구현 세부사항을 언급하지 않도록 오류 메세지가 개선되었습니다.
...
이제 `unwinding panics are not supported without std` 오류가 해결되었을 것입니다.

[abort on panic]: https://github.com/rust-lang/rust/pull/32900

이제 다시 컴파일을 시도하면, 새로운 오류가 뜰 것입니다:
...
구버전 Rust 툴체인에서는 이 오류가 `error: requires start lang_item`으로 나타납니다. `start` language item은 이후에 `main` 함수를 호출하는 실제 실행 시작 지점을 지정합니다. 따라서 이 오류는 신버전 툴체인에서 나타나는 ``using `fn main` requires the standard library`` 오류와 같은 원인을 가집니다.

</div>

## 실행 시작 지점

혹자는 `main` 함수가 프로그램의 "실행 시작 지점 (entry point)", 즉 프로그램 실행 시 가장 먼저 호출되는 함수라고 생각할지도 모릅니다. 하지만 대부분의 프로그래밍 언어들은 [런타임 시스템][runtime system]을 가지고 있는데, 이는 가비지 컬렉션 (예시: Java) 혹은 소프트웨어 스레드 (예시: GoLang의 goroutine) 등의 기능을 담당합니다.
...
우리의 "freestanding 실행 파일" 은 Rust 런타임이나 `crt0`에 접근할 수 없기에, 우리가 직접 실행 시작 지점을 지정해야 하며 단순히 `main` 함수를 정의하는 것만으로는 부족합니다.
