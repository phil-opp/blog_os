...
Теперь компилятору не хватает функции `#[panic_handler]`, и он сообщает, что _раскрутка (unwinding)_ невозможна без стандартной библиотеки. Обе ошибки мы рассмотрим в следующих разделах.

<div class="note">

На более старых версиях Rust-тулчейна вторая ошибка выглядит как `language item required, but not found: eh_personality`, и у неё та же причина, что и у ошибки `unwinding panics are not supported without std`.
...
По умолчанию Rust использует [раскрутку стека][stack unwinding] для запуска деструкторов всех _живых_ переменных на стеке в случае [паники][panic]. Это гарантирует, что вся использованная память будет освобождена, и позволяет родительскому потоку перехватить панику и продолжить выполнение. Однако раскрутка &mdash; очень сложный процесс и требует некоторых специфичных для ОС библиотек (например, [libunwind] для Linux или [structured exception handling] для Windows), которым нужна стандартная библиотека Rust. Как следствие, мы не можем использовать раскрутку для ядра нашей операционной системы с `no_std`.

<div class="note">

Ошибка `language item required, but not found: eh_personality` на более старых версиях Rust-тулчейна тоже указывает на раскрутку. Элемент языка [`eh_personality`][`eh_personality` language item] обозначает функцию, которая должна использоваться для реализации раскрутки стека. Элементы языка &mdash; это специальные элементы (трейты, функции, типы и т.д.), которые внутренне требуются компилятору. На более новых версиях Rust-тулчейна сообщение об ошибке улучшили, чтобы не упоминать эту деталь реализации.
...
Это устанавливает стратегию паники на `abort` (прерывание) как для профиля `dev` (используемого для `cargo build`), так и для профиля `release` (используемого для `cargo build --release`). Теперь ошибка `unwinding panics are not supported without std` должна быть исправлена.

[abort on panic]: https://github.com/rust-lang/rust/pull/32900

Если мы попытаемся скомпилировать программу сейчас, возникнет новая ошибка:
...
На более старых версиях Rust-тулчейна эта ошибка выглядит как `error: requires start lang_item`. Элемент языка `start` определяет базовую точку входа, которая затем вызывает функцию `main`. Таким образом, у этой ошибки та же причина, что и у ошибки ``using `fn main` requires the standard library`` на более новых тулчейнах.

</div>

## Точка входа

Можно подумать, что функция `main` &mdash; это "точка входа" программы, т.е. первая функция, вызываемая при запуске программы. Однако в большинстве языков есть [среда выполнения][runtime system], которая отвечает за такие вещи, как сборка мусора (например, в Java) или программные потоки (например, goroutines в Go). Эта система выполнения должна быть вызвана до `main`, поскольку ей необходимо инициализировать себя.
...
Наш независимый исполняемый файл не имеет доступа к runtime Rust и `crt0`, поэтому нам нужно определить собственную точку входа и мы не можем просто определить функцию `main`.
