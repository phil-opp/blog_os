...
这个配置项的意思是，我们的编译目标不支持 panic 时的**栈展开**（[stack unwinding](https://www.bogotobogo.com/cplusplus/stackunwinding.php)），所以我们选择直接**在 panic 时中止**（abort on panic）。这和在 `Cargo.toml` 文件中添加 `panic = "abort"` 选项的作用是相同的，所以我们可以从 `Cargo.toml` 中移除该选项。（请注意，与 `Cargo.toml` 中的选项不同，这个目标配置项在我们后面重新编译 `core` 库时同样会生效。因此，即使你更倾向于保留 `Cargo.toml` 中的选项，也请务必加上这个配置项。）
...
由于我们想要使用 `soft-float` 特征，我们还需要告诉 Rust 编译器 `rustc` 我们想要使用对应的 ABI。我们可以通过将 `rustc-abi` 配置项设置为 `softfloat` 来实现这一点。
...
要编译我们的新目标平台，我们将使用 Linux 系统的约定，因为 `ld.lld` linker-flavor 会指示 llvm 使用 `-flavor gnu` 标志进行编译（关于更多链接器选项，请参阅 [rustc 文档](https://doc.rust-lang.org/rustc/codegen-options/index.html#linker-flavor)）。这意味着，我们需要把[前一篇文章][previous post]中编写的入口点重命名为 `_start`：
...
[unsafe superpowers]: https://doc.rust-lang.org/stable/book/ch20-01-unsafe-rust.html#unsafe-superpowers
...
为了解决这个问题，我们建议使用 `bootimage` 工具——它将会在内核编译完毕后，将它和引导程序组合在一起，最终创建一个能够引导的磁盘映像。要安装这款工具，请进入你的用户主目录（或任何位于你的 cargo 项目之外的目录），并在终端中执行下面的命令：
...
在安装 `bootimage` 并添加 `llvm-tools-preview` 组件之后，你可以回到你的 cargo 项目目录，并执行下面的命令来创建一个可引导的磁盘映像：
