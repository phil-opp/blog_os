...
`soft-float` 기능을 사용하려면, Rust 컴파일러 `rustc` 에게도 해당 ABI를 사용하겠다고 알려줘야 합니다. 이를 위해 `rustc-abi` 필드를 `softfloat` 으로 설정하면 됩니다.
...
### 커널 빌드하기
우리가 정의한 새로운 컴파일 대상 환경을 목표로 컴파일할 때에 리눅스 시스템의 관례를 따를 것입니다. 이는 `ld.lld` linker-flavor가 llvm에게 `-flavor gnu` 플래그로 컴파일하도록 지시하기 때문입니다 (더 다양한 링커 옵션에 대해서는 [rustc 공식 문서](https://doc.rust-lang.org/rustc/codegen-options/index.html#linker-flavor)를 확인해주세요). 즉, [지난 포스트][previous post]에서 설명한 것처럼 우리는 실행 시작 지점의 이름을 `_start`로 지정할 것입니다:
...
**참고:** 이 포스트는 `bootloader v0.9`에서만 호환됩니다. 최신 버전은 다른 빌드 시스템을 사용하기에 이 포스트를 따라 하면 빌드 오류가 발생합니다.
...
이 문제를 해결하기 위해 저희가 `bootimage` 라는 도구를 만들었습니다. 이 도구는 커널과 부트로더를 각각 컴파일 한 이후에 둘을 링크하여 부팅 가능한 디스크 이미지를 생성해줍니다. 이 도구를 설치하려면 홈 디렉토리 (또는 cargo 프로젝트 외부의 아무 디렉토리)로 이동한 후 터미널에서 아래의 명령어를 실행해주세요.
...
`bootimage` 도구를 설치하고 `llvm-tools-preview` 컴포넌트를 추가하셨다면, 이제 cargo 프로젝트 디렉토리로 돌아가 아래의 명령어를 실행하여 부팅 가능한 디스크 이미지를 만들 수 있습니다:
...
USB 드라이브에 우리의 커널을 저장한 후 실제 컴퓨터에서 부팅하는 것도 가능하지만, 해당 기기의 **모든 데이터가 덮어씌워지기** 때문에 **정확한 기기명을 선택하도록 주의해주세요**:

```
> dd if=target/x86_64-blog_os/debug/bootimage-blog_os.bin of=/dev/sdX && sync
```

`sdX` 대신 여러분이 소지한 USB 드라이브의 기기명을 입력하시면 됩니다.
