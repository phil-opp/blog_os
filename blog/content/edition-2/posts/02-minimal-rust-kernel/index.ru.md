...
Почти все системы x86 имеют поддержку загрузки BIOS, включая более новые машины на базе UEFI, которые используют эмулированный BIOS. Это замечательно, потому что вы можете использовать одну и ту же логику загрузки на всех машинах из прошлого века. Но такая широкая совместимость одновременно является и самым большим недостатком загрузки BIOS, поскольку это означает, что перед загрузкой процессор переводится в 16-битный режим совместимости под названием [реальный режим], чтобы архаичные загрузчики 1980-х годов все еще работали.
...
Поскольку мы хотим использовать функцию `soft-float`, нам также нужно сообщить компилятору Rust `rustc`, что мы хотим использовать соответствующий ABI. Мы можем сделать это, установив поле `rustc-abi` в значение `softfloat`.
...
Компиляция для нашей новой целевой платформы будет использовать соглашения Linux, поскольку linker-flavor `ld.lld` указывает llvm компилировать с флагом `-flavor gnu` (о других опциях компоновщика см. [документацию rustc](https://doc.rust-lang.org/rustc/codegen-options/index.html#linker-flavor)). Это означает, что нам нужна точка входа с именем `_start`, как описано в [предыдущем посте][previous post]:
...
### Создание загрузочного образа {#creating-a-bootimage}
...
**Примечание:** Этот пост совместим только с `bootloader v0.9`. Более новые версии используют другую систему сборки и приведут к ошибкам сборки при следовании этому посту.
...
Для решения этой проблемы мы создали инструмент `bootimage`, который сначала компилирует ядро и загрузчик, а затем соединяет их вместе для создания загрузочного образа диска. Чтобы установить инструмент, перейдите в свой домашний каталог (или любой каталог за пределами вашего проекта cargo) и выполните следующую команду в терминале:
...
После установки `bootimage` и добавления компонента `llvm-tools-preview` вы можете создать загрузочный образ диска, вернувшись в каталог вашего проекта cargo и выполнив команду:
...
Откроется отдельное окно, которое должно выглядеть примерно так:
