...
`soft-float`機能を使いたいので、対応するABIを使いたいということをRustコンパイラ`rustc`にも伝える必要があります。これは、`rustc-abi`フィールドを`softfloat`に設定することで行えます。
...
### カーネルをビルドする
私達の新しいターゲットのコンパイルにはLinuxの慣習に倣います。というのも、`ld.lld`というlinker-flavorはllvmに`-flavor gnu`フラグをつけてコンパイルするよう指示するからです（リンカのオプションについてより詳しくは、[rustcのドキュメント](https://doc.rust-lang.org/rustc/codegen-options/index.html#linker-flavor)をご覧ください）。つまり、[前の記事][previous post]で説明したように`_start`という名前のエントリポイントが要るということです。
...
**注意:** この記事は`bootloader v0.9`にのみ対応しています。より新しいバージョンは異なるビルドシステムを使っているため、この記事の通りに進めるとビルドエラーが発生します。
...
この問題を解決するため、私達は`bootimage`というツールを作りました。これは、まずカーネルとブートローダーをコンパイルし、そしてこれらをリンクしてブータブルディスクイメージを作ります。このツールをインストールするには、ホームディレクトリ（もしくはcargoプロジェクトの外にある任意のディレクトリ）に移動し、以下のコマンドをターミナルで実行してください：
...
`bootimage`をインストールし、`llvm-tools-preview`を追加したら、cargoプロジェクトのディレクトリに戻り、以下のように実行することでブータブルディスクイメージを作れます：
...
USBメモリにこれを書き込んで実際のマシン上で起動することも可能です。ただし、そのデバイス上のすべてのデータが上書きされてしまうので、 **正しいデバイス名を選んでいるのかよく確認してください** ：

```
> dd if=target/x86_64-blog_os/debug/bootimage-blog_os.bin of=/dev/sdX && sync
```

`sdX`はあなたのUSBメモリのデバイス名です。
